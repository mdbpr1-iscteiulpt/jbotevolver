...
public class StayAwayFromEachOtherEvaluationPlusCatchPreyFunction extends EvaluationFunction{
	...
	public StayAwayFromEachOtherEvaluationPlusCatchPreyFunction(Arguments args) {
		super(args);	
	}

	//@Override
	public double getFitness() {
		return (fitness - numberOfRobotsThatCollided);	/*numberOfRobots + numberOfRobotsDistanced + numberOfRobotsDanger))*/
	}
	...
	public void update(Simulator simulator) {			
		...
		for(Robot r : simulator.getEnvironment().getRobots()){
			...
			if(r instanceof FlightDroneRobot) {
				if(((FlightDroneRobot) r).getForwardSpeed()/*+((FlightDroneRobot) r).getFlightSpeed()*/ < 0.2)
					robotsBellowSpeed++;
			}
		...
		fitness += (double) numberOfRobotsDistanced * 0.001 + robotsBellowSpeed * -0.0001 + numberOfRobotsDanger * -0.0001;
	}
}